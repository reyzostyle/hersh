...
import { scoreHook, MAX_HOOK_CHARS } from '../_shared/text-scoring.ts';
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (hook.length > MAX_HOOK_CHARS) {
      return new Response(JSON.stringify({ error: `Hook too long (max ${MAX_HOOK_CHARS} chars)` }), { status: 400, headers: CORS });
    }
    ...
    let result;
    try {
      result = await scoreHook(supabase, userId, hook, context);
    } catch (e) {
