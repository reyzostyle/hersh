...
export function VideoUploadPanel({ open, onClose, onAnalyze, analyzing, uploadStep }: VideoUploadPanelProps) {
  ...
  useEffect(() => {
    if (!open) { setFile(null); setVideoContext(''); setFileError(''); }
  }, [open]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        {/* Body */}
        <div className="flex-1 overflow-y-auto flex flex-col p-6 gap-5">
          {/* Drop zone */}
          <div>
            <input
              ref={fileInputRef}
            ...
            {!file ? (
              ...
              <div className="rounded-xl px-4 py-3 flex items-center gap-3" style={{ background: 'rgba(52,211,153,0.08)', border: '1px solid rgba(52,211,153,0.2)' }}>
                ...
                <button onClick={() => { setFile(null); setFileError(''); }} className="p-1 text-gray-500 hover:text-white rounded transition-colors">
                  <X className="w-4 h-4" />
                </button>
              </div>
            )}
